...
RSpec.describe TasksController, type: :controller do
  ...
  describe "tasks#update" do
    it "should allow tasks to be marekd as done" do
  ...
  describe "tasks#create" do 
    it "should allow new tasks to be created" do
      post :create, params: {task: {title: "Meal Prep"}}
      expect(response).to have_http_status(:success)
      response_value = ActiveSupport::JSON.decode(@response.body)
      expect(response_value['title']).to eq("Meal Prep")
      expect(Task.last.title).to eq("Meal Prep")
    end
  end
